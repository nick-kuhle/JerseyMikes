...
- **Volume.** A busy block carries ~300–400 transactions. Each is run through
  the strategy pre-filters (cheap) and only candidates reach the simulator, so
  the steady-state cost is one `eth_getBlockByHash` per block plus the usual
  simulation load.
